...
- [ ] **TradingView open-source migration** (verified against TV docs 2026-07-30: a published script's privacy/visibility are PERMANENT — update dialog greys them out; no conversion path exists):
  ...
  - [ ] Title the new listing exactly "TheStrat Suite" — no "[Open Source]" suffix (TV badges open scripts itself; the new listing is the canonical product). No version numbers in the title.
  - [ ] Old listing's title is IMMUTABLE (confirmed by attempt 2026-07-30 — titles lock at publish like visibility). Mark it legacy via content instead: (1) Author's instructions → "now free and open source, this listing is legacy, existing users keep access, current version = 'TheStrat Suite' by SpinTrades / TheStratSuite.com"; (2) description lead paragraph with the new listing's link; (3) one final Update whose release notes point to the new listing. TV's INVITE-ONLY vs OPEN-SOURCE badges disambiguate the two listings in search regardless.
