...
|---|---|---|
| Lines | 60 | 10 owned line fields per `TimeframeData` slot (trigger high/low, open, magnitude x2, exhaustion x2, stop x2, F2 open) x 6 slots |
| Boxes | 12 | 2 Take Action Window boxes per slot x 6 |
| Labels | ~120 | <=10 level labels per slot before consolidation, x 2 render modes (timeline + floating) |
