...
- **Compact Cell Color** (default: Signal | options: Signal / Timeframe Continuity) - What the colored timeframe cells mean in Compact mode; greyed out in Full mode. **Signal** colors by setup state: green or red when a signal is in force in that direction, yellow when the bar is inside and above its open, orange when inside and below, plus the outside-bar and Failing 2 colors. **Timeframe Continuity** ignores signals and colors purely by direction - green when that timeframe is trading above its open, red when below - using the same comparison the FTFC calculation applies to each slot. So the row becomes the per-timeframe breakdown of the FTFC summary underneath it: all green or all red is full continuity, a mixed row is the conflict. New in v3.0.1 (`TABLE-COMPACT-COLOR-1`).
