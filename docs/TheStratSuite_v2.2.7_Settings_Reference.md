...
- **Break Even at Magnitude** (default: off | options: on / off) - When price reaches the magnitude (first target) level, the stop moves to the entry trigger, so the remaining risk at that level is zero (subject to slippage and gaps). If the signal has no magnitude level, it uses the exhaustion level as the move-to-break-even trigger instead. Turn this on to move the stop to break even once the first target is reached; leave it off if you prefer to let the original stop ride to its structural invalidation.
