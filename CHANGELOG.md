...
Repository scope. Nothing in this section changes the TheStrat Suite indicator itself.

### Added

- `pine/tools/` - sibling tools, each shipping as its own script rather than inside the
  Suite. **Price Action Invalidation Stop Loss** (`PA_Invalidation_Stop.pine`), a structural
  stop placed at the prior completed 30m/60m level with six selectable versions; and
  **PowerBar** (`PowerBar.pine`), which marks candles that are nearly all body relative to a
  session-aware, time-of-day-adjusted expected range. Documented in `docs/tools/`.
- `core/` - `strat_core`, the canonical definitions as a dependency-free Python package:
  bar-state classification (the `structure` / `above_open` / `failed` / `live` tuple),
  combo strings, timeframe continuity, hammer/shooter detection, and the PowerBar core
  shape. Standard library only, with a unit-test suite pinning the strict-vs-inclusive
  comparison rules that a port is most likely to get wrong.

### Removed

- The Volume Profile research track (`pine/research/` and `diagnostics/volume-profile-phase0/`).
  It was exploratory material that had not earned its way into a shipped tool, and it is out
  of scope for this repository.

### Changed

- Documentation pass for public readability: typography normalized to plain ASCII (the
  indicator's own on-chart glyphs are preserved as-is), internal release and listing
  runbooks removed, and reviewer attributions dropped from `FIX` comments. No code
  behavior changed.
